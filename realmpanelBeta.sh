...
install_panel() {
    ...
    if [ -x "$(command -v apt-get)" ]; then
        ...
        apt-get install -y python3 nftables >/dev/null 2>&1
    elif [ -x "$(command -v yum)" ]; then
        yum install -y python3 nftables >/dev/null 2>&1
    fi
...
def sync_nftables():
    nodes = read_config()
    os.system("nft add table inet realm_table 2>/dev/null")
    os.system("nft add chain inet realm_table input_hook { type filter hook input priority 0 \\; } 2>/dev/null")
    os.system("nft add chain inet realm_table output_hook { type filter hook output priority 0 \\; } 2>/dev/null")
    os.system("nft add chain inet realm_table realm_acct 2>/dev/null")
    
    os.system("nft flush chain inet realm_table input_hook 2>/dev/null")
    os.system("nft add rule inet realm_table input_hook jump realm_acct 2>/dev/null")
    os.system("nft flush chain inet realm_table output_hook 2>/dev/null")
    os.system("nft add rule inet realm_table output_hook jump realm_acct 2>/dev/null")
    
    os.system("nft add chain inet realm_table realm_drop { type filter hook input priority -1 \\; } 2>/dev/null")
    os.system("nft add set inet realm_table disabled_tcp { type inet_service \\; } 2>/dev/null")
    os.system("nft add set inet realm_table disabled_udp { type inet_service \\; } 2>/dev/null")
    os.system("nft flush chain inet realm_table realm_drop 2>/dev/null")
    os.system("nft add rule inet realm_table realm_drop tcp dport @disabled_tcp drop 2>/dev/null")
    os.system("nft add rule inet realm_table realm_drop udp dport @disabled_udp drop 2>/dev/null")
    
    os.system("nft flush chain inet realm_table realm_acct 2>/dev/null")
    for n in nodes:
        try:
            p = int(n['inPort'])
            os.system(f"nft add rule inet realm_table realm_acct tcp sport {p} counter 2>/dev/null")
            os.system(f"nft add rule inet realm_table realm_acct udp sport {p} counter 2>/dev/null")
            os.system(f"nft add rule inet realm_table realm_acct tcp dport {p} counter 2>/dev/null")
            os.system(f"nft add rule inet realm_table realm_acct udp dport {p} counter 2>/dev/null")
        except: pass
    ...
    while True:
        ...
        try:
            ...
            if data.get("month") != current_month:
                ...
                # 月初清空规则
                os.system("nft flush set inet realm_table disabled_tcp 2>/dev/null") 
                os.system("nft flush set inet realm_table disabled_udp 2>/dev/null") 
            ...
            for p_str, b in data["nodes"].items():
                ...
                if q_gb > 0 and b >= q_gb * 1073741824:
                    if p_str not in disabled_list:
                        os.system(f"nft add element inet realm_table disabled_tcp {{ {p_str} }} 2>/dev/null")
                        os.system(f"nft add element inet realm_table disabled_udp {{ {p_str} }} 2>/dev/null")
                        data.setdefault("disabled", []).append(p_str)
                else:
                    if p_str in disabled_list:
                        os.system(f"nft delete element inet realm_table disabled_tcp {{ {p_str} }} 2>/dev/null")
                        os.system(f"nft delete element inet realm_table disabled_udp {{ {p_str} }} 2>/dev/null")
